...
- [x] **评测体系**：25 道黄金评测集分 7 层，检索命中率与答案覆盖率分开统计
- [x] **商品名归一化**：借 `kb_item_names` 集合把口语商品名映射为库中规范名，
      阈值 0.70 由实测相似度分布确定
- [x] **模型单例线程安全**：embedding 与 reranker 改为双重检查加锁，
      修复并发下的 meta tensor 竞态
- [x] **推理调用串行化**：修复 GPU + FP16 + 并发三者同时满足时的 dtype 竞态
- [x] **GPU 推理支持**：准确率持平的前提下 P50 延迟由 50.7s 降至 4.5s
- [ ] **跨设备归纳**：识别「问的是一类设备」并跳过单设备过滤
- [ ] **附录章节召回**：L1-04 暴露的召回短板，需调整切分或检索策略
